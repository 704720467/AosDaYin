...
public class AosHuoQianDaYinActivity extends Activity {
    ...
    private boolean isPrintFinish = false;//记录打印的状态，false打印失败，true ：打印成功
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 2://打印机打印成功
                    ...
                    //                    for (int i = 0; i < 10; i++)
                    //                        Toast.makeText(AosHuoQianDaYinActivity.this, "打印成功，打印成功！", Toast.LENGTH_LONG).show();
                    //                    if (currentPrintPosition == iCount - 1) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.aos_huoqiandayin);
        init();
    }
    ...
    private void init() {
        ...
        submit_btn.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(final View v) {
                ...
                if (isPrintFinish) {
                    Toast.makeText(AosHuoQianDaYinActivity.this, "当前运单号下的转运路由标签本次打印已完成，如需再次打印，请退出当前界面重新进入！", Toast.LENGTH_LONG).show();
                    return;
                }
                synchronized (this) {
                    tipPrint();
    ...
    private void toPrintNext() {
        ...
        try {
            for (String s : huobhlist) {
                ...
                //            Bitmap b = MyBarcode.QS_02F(texts);
                //            MyBarcode.saveBitmap("第二个标签" + currentPrintPosition, b);
                //            currentPrintPosition++;
                //            if (currentPrintPosition > 1)
                //                break;
                PrintUtil.toPrint(AosHuoQianDaYinActivity.this, MyBarcode.QS_02F(texts), handler);
            }
            isPrintFinish = true;
            SaveServerData();
        } catch (Exception e) {
    ...
    private void toShowFristImage() {
        ...
        Texts.add(createDate);
        Texts.add(createTime);
        fristMap = MyBarcode.QS_02F(Texts);
    ...
    private void tipPrint() {
        Builder builder = new Builder(AosHuoQianDaYinActivity.this);
        String tip = (printCount.equals("0")) ? "是否进行打印？" : "当前打印为第（" + printCount + "）次，可能存在识别风险，请确认是否继续打印？";
        if (isPrintFinish)
            tip = "当前运单号所属转运路由标签打印已完成，无法再次打印，如需再次打印，退出后重新进入!";
        builder.setTitle("提示:");
        ...
        builder.setPositiveButton(isPrintFinish ? "确定" : "取消", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                dialog.dismiss();
            }
